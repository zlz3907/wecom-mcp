...
- `SMART_SHEETS_IDS` 创建与完整实例初始化都必须执行同一完成契约：恰好20个标准字段、清理可证明归属且完全匹配平台默认模板的空字段/空记录、复用主字段、以独立保留键登记索引表自身，并完整回读唯一自登记与字段摘要后才报告完成。已有 docid 或曾经 verified 不能跳过核验；详见 `REGISTRY_INITIALIZATION_CONTRACT.md`。
