...
5. macOS/Linux 上的 Codex 与 TRAE SOLO CN 只使用安装器内置的最小、备份后、幂等注册器。Windows 上通过客户端官方 MCP 设置入口注册包内 `bin/wecom-mcp-v2.exe`，参数为 `--config` 和既有配置文件绝对路径；修改配置文件前先备份，只合并 `zoop_wecom_zhycit` 并保留其他服务。WorkBuddy 同样使用官方“设置 > MCP”入口或官方 `mcp.json` 编辑器。不要猜配置路径，不要覆盖整个配置；无法确认客户端契约时输出 agent_blocked。
