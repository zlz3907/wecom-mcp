...
2. 从 https://github.com/zlz3907/wecom-mcp/releases/latest 解析重定向得到固定 release tag；下载该 tag 的 install.sh 与 SHA256SUMS，先校验 install.sh 的 SHA-256，再执行它。不得执行 latest URL 直接给出的二进制，也不得从对话历史猜测二进制路径。
...
evidence=<release checksum, installer output, and runtime call result>
next_action=<one concrete action>

在权限不足、缺少 SHA-256 工具、私有 Release 无访问权、无本地配置、未知客户端契约或平台不支持时，停止并输出 result=agent_blocked；不得删除旧版本、不得伪造成功、不得重试不确定的写入。
```
