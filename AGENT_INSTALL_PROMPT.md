...
2. 从 https://github.com/zlz3907/wecom-mcp/releases/latest 解析重定向得到固定 release tag；只从该固定 tag 下载 install.sh、RELEASE-MANIFEST.txt 与 SHA256SUMS。仓库为 Private 时，只使用当前 Agent 已有的 GitHub 登录态或 `GH_TOKEN`/`GITHUB_TOKEN`，通过临时受保护认证文件访问，绝不输出、保存或复制 token。先用 SHA256SUMS 校验 install.sh 和 RELEASE-MANIFEST.txt，再确认 manifest 的 version、installer、checksums 与当前 tag 一致，最后执行已校验的 install.sh。不得执行 latest URL 直接给出的二进制，也不得从对话历史猜测二进制路径。
