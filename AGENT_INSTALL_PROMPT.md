...
4. 安装后由 Agent 自动发现组织配置，普通用户不提供、选择、复制、移动或确认任何路径：
   ...
   - 找到实例配置后，只验证它是绝对路径下的可读普通文件且非链接，JSON 结构有效、`instance_name=zoop_wecom_zhycit`；同时确认 `wecom_operator_userid` 是非占位的固定企微 userid，`instance_initialize` capability group 含 `list_employees` 与 `grant_doc_readers`，记录写入 capability group 含 `list_employees`。这些检查只报告 present/missing，不输出 userid、配置内容、doc ID 或 sheet ID。从配置内部自动读取 `schema_mirror_path` 并验证其为绝对路径下的可读普通文件和有效 JSON；不得再次要求用户提供 Schema 路径。只确认三项 GNAS 运行环境是否已由组织持久化配置，不输出或复制其值，不输出配置内容、凭据、token 或 secret。
   - 未找到配置，或缺少 Schema、三项 GNAS 环境、`wecom_operator_userid`、上述 operator capability 时，二进制安装仍可为 `installed=yes`，但不得把实例误报为可初始化。面向普通用户只输出：“程序已经安装，但组织配置尚未部署或版本过旧，请联系本组织技术人员更新 GNAS 本机配置包；部署后重新粘贴原安装指令即可。”不得列出参数让用户收集，不得要求用户把文件放到某处、告诉 Agent 路径、粘贴凭据或执行命令。
