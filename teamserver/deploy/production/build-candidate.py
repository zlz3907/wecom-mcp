...
def main():
    ...
    for key in ('expected-runtime-path','expected-binary-sha256','expected-unit-fingerprint','expected-runtime-config-fingerprint','expected-gnas-release-id','expected-gnas-binary-sha256','ci-url'):
    ...
    for digest in (a.expected_binary_sha256, a.expected_unit_fingerprint, a.expected_runtime_config_fingerprint, a.expected_gnas_binary_sha256):
    ...
    out.chmod(0o500)
    print(json.dumps({'state':'candidate','release_id':rid,'binary_sha256':manifest['files']['wecom-mcp-team'],'manifest_sha256':c.sha(out/'manifest.json'),'production_changed':False}))
