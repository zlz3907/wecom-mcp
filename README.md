...
`wecom_schema_registry_status` 只读返回 Z-S00 当前 `@active` 指针、generation 完整性和字段条目数量。`wecom_schema_registry_update` 不接受调用方提供的文档、子表、字段或类型；它只从当前固定实例的 Z-S01 至 Z-S09 读取真实结构，写入带 manifest 的不可变 generation，完整回读后才以单条 `@active` 指针切换生效。输入必须包含 status 返回的 `expected_active_generation`（尚无 active 时为 `none`）、独立幂等键、来源修订和固定 Owner 授权 `refresh_online_schema_registry`。并发期望不一致、同键内容冲突、generation 缺行或回读不完整都会失败关闭，不会切换 active。
